...
=======
      const fileExt = file
        ? file.name.split(".").pop()
        : selectedAward?.filename.split(".").pop();
      const timestamp = Date.now();
      const fileName = `${text.filename}_${timestamp}.${fileExt}`;
      const randomid = crypto.randomUUID();
      const filePath = selectedAward
        ? `${selectedAward.id}/${fileName}`
        : `${randomid}/${fileName}`;
>>>>>>> f59dd2f (update Analytics)
...
<<<<<<< HEAD
=======
            awardranking: text.awardranking
>>>>>>> f59dd2f (update Analytics)
          })
            ...
            <select
              ...
              onChange={(e) => {
                if (e.target.value === "new") {
                  ...
                  setSelectedAward(award || null);
                  setPreview(
                    `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/quantimages/${award?.id}/${award?.filename}` ||
                      null
                  );
                  setText({
                    awardsname: award.awardsname,
            ...
            >
              ...
              {awards.map((award) =>
                award && award.awardsname ? (
                  <option
                    key={award.id}
                    value={`${award.awardsname}-${award.description}`}
                  >
                    {award.awardsname} - {award.description} {award.awardsyear} 
                  </option>
                ) : null
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                type="text"
                name="awardranking"
                defaultValue={selectedAward?.awardranking}
                onChange={onChangeHandler}
